...
[CardFx(typeof(MegaCrit.Sts2.Core.Models.Cards.ShiningStrike))]
public class ShiningStrike : CardFX {
    ...
    private async Task PlayVfx(Creature owner, Creature target) {
        ...
        try {
            ...
            vfxNode.Scale *= 1.8f;
            if (VFXUtil.RandRD(0.4f)) vfxNode.Scale *= new Vector2(1f, -1f);
            // 计算长矛射出方向（从玩家指向目标）
            ...
            Entry.Logger.Warn($"[ShiningStrike] Error playing VFX: {ex.Message}");
        }
    }
}